...
def fetch_fallback_local(endpoint: str, enctoken: str) -> Dict[str, Any]:
    ...
    if "advisory" in endpoint:
        from src.services.llm_advisor import get_recommendations
        import urllib.parse as _up
        qs = endpoint.split("?", 1)[1] if "?" in endpoint else ""
        params = dict(_up.parse_qsl(qs))
        return {"status": "success", **get_recommendations(
            enriched,
            investment_goal=params.get("investment_goal", "Moderate Growth"),
            max_single_stock_pct=float(params.get("max_single_stock_pct", 15.0)),
            max_sector_pct=float(params.get("max_sector_pct", 25.0)),
        )}
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### AI Advisory Settings")
    investment_goal = st.selectbox(
        "Investment goal",
        ["Moderate Growth", "Aggressive Growth", "Capital Preservation", "Income / Dividend", "Balanced"],
    )
    max_stock_cap = st.slider("Max single stock allocation (%)", 5, 40, 15)
...
advisory_res, _ = fetch_from_backend(
    f"advisory/recommendations?investment_goal={investment_goal}&max_single_stock_pct={max_stock_cap}&max_sector_pct={max_sector_cap}",
...
if tab5.open:
    with tab5:
        st.subheader("🤖 AI Portfolio Advisory")

        adv_source = advisory_res.get("source", "rule_engine")
        adv_provider = advisory_res.get("llm_provider")
        adv_goal = advisory_res.get("investment_goal", investment_goal)
        rule_flags = advisory_res.get("rule_flags", [])
        recs = advisory_res.get("recommendations", [])

        # Source badge
        if adv_source == "llm" and adv_provider:
            st.success(f"✨ Recommendations generated by **{adv_provider}** · Goal: *{adv_goal}*")
        else:
            st.info("ℹ️ Showing **rule-based recommendations** (LLM unavailable or not configured). Set `GEMINI_API_KEY` in `.env` to enable AI advisory.")

        # Rule engine flags
        if rule_flags:
            st.markdown("#### ⚠️ Rule Engine Alerts")
            for flag in rule_flags:
                severity = flag.get("severity", "MEDIUM")
                icon = "🔴" if severity == "HIGH" else "🟡"
                st.warning(f"{icon} **{flag.get('rule', '')}** — {flag.get('detail', '')}")
        else:
            st.success("✅ No rule violations detected in current portfolio.")
        ...
        if not recs:
            ...
            ACTION_COLORS = {"BUY": "🟢", "HOLD": "🔵", "TRIM": "🟡", "SELL": "🔴"}

            for rec in recs:
                action = rec.get("action", "HOLD")
                icon = ACTION_COLORS.get(action, "⚪")
                conf = rec.get("confidence_score", 0.0)
                with st.expander(f"{icon} **{rec.get('symbol')}** — {action}  ·  Target: {rec.get('target_allocation_pct', 0):.1f}%  ·  Confidence: {conf:.0%}"):
            ...
            st.dataframe(
                pd.DataFrame(recs)[[c for c in ["symbol", "action", "target_allocation_pct", "confidence_score", "rationale", "source"] if c in pd.DataFrame(recs).columns]],
                column_config={
                    "symbol": st.column_config.TextColumn("Symbol", pinned=True),
                    "action": st.column_config.TextColumn("Action"),
                    "target_allocation_pct": st.column_config.NumberColumn("Target alloc (%)", format="%.1f%%"),
                    "confidence_score": st.column_config.ProgressColumn("Confidence", min_value=0.0, max_value=1.0, format="%.0%%"),
